...
def pack_response():
        ...
        response['time'] = frequency_var.get()
        response['function'] = function_var.get()
        response['store'] = store_var.get()
        response['maxsize'] = maxsize_var.get()
        response['folder'] = folder_var.get()
        return response
...
ttk.Label(mainframe, text="File to back up").grid(row=2, column=5)
